...
// @namespace   saoew
// @include     /http:\/\/a57528\.app\.gree-pf\.net\/sp_web\.php\?(guid=ON&)?action_home_info_item_equipment_setlist=true(&step=updateSetEquip)?(&guid=ON)?(&tab=[12])?(&deckCode=[1-5])?(&itemDivCode=10\d&subDiv=1&itemCode=\d+&eid=\d+)?&opensocial_owner_id=\d+/
// @include     /http:\/\/a57528\.app\.gree-pf\.net\/sp_web\.php\?action_home_info_item_list=true&guid=ON(&rareKey=\d+)?(&sortType=[a-z]+(_des)?)?(&equipDiv=(0|10\d))?(&isLimitStone=[01])?(&itemCat=1)?(&start=\d+)?&opensocial_owner_id=\d+/
...
// @include     /http:\/\/a57528\.app\.gree-pf\.net\/sp_web\.php\?action_home_info_item_equipment_setchange=true&guid=ON(&eid=)?&deckCode=[1-5](&itemDivCode=10\d)?&subDiv=[01](&sortKey=[a-z]+(_des)?)?(&rareRank=)?(&inKey=1)?(&start=\d+)?&opensocial_owner_id=\d+/
